...
export async function POST(request: Request) {
  try {
    ...
    const subject = fields.subject || "New Inquiry";
    const message = fields.message || "";
    const type = fields.type || "Contact Request"; // "Booking Request" or "Contact Request"

    // Optional fields for backward compatibility or specific forms
    ...
    const html = `
      ...
      ${phone ? `<p><strong>Phone:</strong> ${String(phone)}</p>` : ""}
